...
<select id="cameraSelect" style="margin: 5px 0; display: none;"></select>

<script input="submit" default="Load Camera">
  // beim Laden: verfügbare Kameras ermitteln (falls schon Berechtigung da ist)
  async function initCameras() {
    ...
    try {
      ...
      // wenn keine Kamera: Select ausblenden
      if (videoDevices.length === 0) {
        select.style.display = "none";
        return;
      }

      // vorhandene Optionen leeren
      select.innerHTML = "";

      videoDevices.forEach((device, index) => {
        const option = document.createElement("option");
        option.value = device.deviceId;
        // label kann leer sein, wenn noch keine Berechtigung erteilt wurde
        option.text = device.label || `Kamera ${index + 1}`;
        select.appendChild(option);
      });

      // nur anzeigen, wenn mehr als 1 Kamera da ist
      select.style.display = videoDevices.length > 1 ? "block" : "none";
    } catch (err) {
      console.log("Fehler beim Auflisten der Geräte:", err);
    }
  }

  // einmal beim Laden versuchen
  initCameras();
  "Loaded Cameras"
</script>
...
if (video.srcObject === null) {
    // Kamera öffnen
    // ausgewählte Kamera-ID holen (kann leer sein, dann nimmt der Browser die Standardkamera)
    ...
    if (navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia(constraints)
            .then(async function (stream) {
                ...
                // Jetzt, wo Berechtigung erteilt ist, Kameraliste aktualisieren
                await initCameras()
